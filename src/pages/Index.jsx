...
import { useUpdateServiceUsage } from '../integrations/supabase/hooks/useServiceUsage';
...
const Index = () => {
  ...
  const updateServiceUsage = useUpdateServiceUsage();
    ...
    try {
      ...
      await Promise.all(
        selectedServices.map(service => 
          updateServiceUsage.mutateAsync({ serviceName: service.name })
        )
      );
      ...
      toast.success("サービスの使用を記録しました");
    } catch (error) {
      console.error('Failed to record service usage:', error);
      toast.error("サービスの使用記録に失敗しました");
    }
  };
  ...
  const getServicePlan = (serviceName) => {
    if (!servicePlans) return null;
    const planData = servicePlans.find(p => p.service_name === serviceName);
    ...
    <Layout>
      <div className="flex h-full">
            ...
            <div key={genre} className="mb-6">
              ...
              <div className="space-y-2">
                {services.map(service => {
                  const plan = getServicePlan(service.name);
                  return (
                    <div
                      key={service.id}
                      ...
                      <span className="flex items-center flex-grow">
                        ...
                        {plan && plan !== 'free' && (
                          <Badge variant="outline" className="ml-2 text-xs text-gray-500">
                            サブスク：{plan === 'basic' ? 'ベーシック' : 'プロプラン'} (${PLAN_PRICES[plan]}/m)
                          </Badge>
                        )}
                      </span>
        ...
        <div className="w-1/2 p-4 bg-white">
          ...
          <Button onClick={openAllServices} className="mt-4 flex items-center gap-2">
